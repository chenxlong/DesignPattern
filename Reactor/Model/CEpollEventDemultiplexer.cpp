/**
 * Project Untitled
 */

#include <assert.h>
#include "CEpollEventDemultiplexer.h"
#include <vector>
#include <sys/epoll.h>

/**
 * CEpollEventDemultiplexer implementation
 */


CEpollEventDemultiplexer::CEpollEventDemultiplexer() {
    m_epoll_fd = ::epoll_create(FD_SETSIZE);
    assert(m_epoll_fd != -1);
    m_fd_num = 0;
}
CEpollEventDemultiplexer::~CEpollEventDemultiplexer()
{
    ::close(m_epoll_fd);
}

/**
 * @param handlers
 * @param timeout
 * @param evnet_timer
 * @return int
 */
int CEpollEventDemultiplexer::WaitEvents(std::map<handle_t, CEventHandler *> * handlers, int timeout, time_heap * evnet_timer) {
    //  申请用于保存已发送事件的 epoll_event 。
    std::vector<epoll_event> ep_evts(m_fd_num);
    //  获取已发生事件的 epoll_event 。
    int num = epoll_wait(m_epoll_fd, &ep_evts[0], ep_evts.size(), timeout);
    if (num > 0)
    {
        for (int idx = 0; idx < num; ++idx)
        {
            handle_t handle = ep_evts[idx].data.fd;
            assert(handlers->find(handle) != handlers->end());
            if ((ep_evts[idx].events & EPOLLERR) || (ep_evts[idx].events & EPOLLHUP)) {
                (*handlers)[handle]->HandleError();
            }
            else
            {
                /**
                * 调用该句柄对应的事件处理器的读写函数。
                */
                if (ep_evts[idx].events & EPOLLIN)
                {
                    (*handlers)[handle]->HandleRead();
                }
                if (ep_evts[idx].events & EPOLLOUT)
                {
                    (*handlers)[handle]->HandleWrite();
                }
            }
        }
    }

//    if (event_timer != nullptr)
//    {
//        event_timer->tick();
//    }

    return num;
}

/**
 * @param handle
 * @param evt
 * @return int
 */
int CEpollEventDemultiplexer::RequestEvent(handle_t handle, event_t evt)
{
    epoll_event ep_evt;
    ep_evt.data.fd = handle;
    ep_evt.events = 0;

    if (evt & kReadEvent)
    {
        ep_evt.events |= EPOLLIN;
    }
    if (evt & kWriteEvent)
    {
        ep_evt.events |= EPOLLOUT;
    }
    ep_evt.events |= EPOLLONESHOT;

    if (epoll_ctl(m_epoll_fd, EPOLL_CTL_MOD, handle, &ep_evt) != 0)
    {
        if (errno == ENOENT)
        {
            if (epoll_ctl(m_epoll_fd, EPOLL_CTL_ADD, handle, &ep_evt) != 0)
            {
                return -errno;
            }
            ++m_fd_num;
        }
    }
    return 0;
}

/**
 * @param handle
 * @return int
 */
int CEpollEventDemultiplexer::UnrequestEvent(handle_t handle) {
    epoll_event ep_evt;
    if (epoll_ctl(m_epoll_fd, EPOLL_CTL_DEL, handle, &ep_evt) != 0)
    {
        return -errno;
    }
    --m_fd_num;
    return 0;
}
